    ...
    <div className="min-h-screen bg-background pt-20 pb-16">
      <div className="container mx-auto px-4 max-w-4xl">
        ...
        <Card className="p-8 bg-gradient-card">
          <h2 className="text-2xl font-semibold mb-6">{questions[currentQuestion]?.questionText}</h2>
          <div className="grid grid-cols-1 gap-3">
            {[questions[currentQuestion]?.option1, questions[currentQuestion]?.option2, questions[currentQuestion]?.option3, questions[currentQuestion]?.option4].map((opt, i) => (
              <Button 
                key={i} 
                onClick={() => handleAnswer(i)}
                className={`w-full justify-start h-auto py-4 px-6 text-left border ${showResults[questions[currentQuestion].questionID] ? 'opacity-50 cursor-not-allowed' : ''}`}
              >
                {opt}
              </Button>
            ))}
