            ...
            <div>
              ...
              <div className="space-y-2">
                {players.map((playerId) => (
                  <div key={playerId} className="flex items-center gap-3 bg-background/50 rounded-lg p-3">
                    <div className="w-10 h-10 bg-primary rounded-full flex items-center justify-center text-white font-semibold">
                      {playerId === 101 ? 'You' : 'P'}
                    </div>
                    <div className="flex-1">
                      <div className="font-medium">Student {playerId}</div>
                      <div className="text-xs text-muted-foreground">
                        {playerId === 101 ? 'Ready' : 'Joined'}
                      </div>
                    </div>
                    {/* Placeholder check for host - assuming 101 is host for now or use isHost logic */}
                    {playerId === 101 && isHost && <Badge variant="secondary">Host</Badge>}
                  </div>
                ))}
              </div>
            </div>
